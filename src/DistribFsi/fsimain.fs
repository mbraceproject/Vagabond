...
let StartServer (fsiSession : FsiEvaluationSession) (fsiServerName : string) = 
    let server =
...
let prepareFsiInteractiveSession (argv : string [], inReader:TextReader, outWriter:TextWriter, errorWriter: TextWriter) =
    ...
    // use custom version of fsi aux library, need to explicitly load dependency
    let argv = Array.append argv [| "-r:DistribFsi.Lib.dll" |]
    ...
    let rec fsiConfig = 
        { // Update the configuration to include 'StartServer' and 'OptionalConsoleReadLine'
            ...
            member __.UseFsiAuxLib = false // use custom version of fsiAux
            member __.PublishInteractionASTs = true
            member __.CustomBanner = fsiConfig0.CustomBanner
