...
//    open Microsoft.FSharp.Control
    ...
    type Vagrant private () =

        static let compiler : (SliceCompilationServer * DependencyExporter) option ref = ref None
        static let loader : DependencyLoader option ref = ref None

        static let 

        static member GetNextDynamicAssemblySlice(assembly : Assembly) =
            match compiler.Value with
            | None -> invalidOp "Vagrant: no assembly compiler has been declared"
            | Some (compiler, exporter) ->
                if not assembly.IsDynamic then invalidArg assembly.FullName "assembly is not dynamic."

                let slices = compiler.CompileNextSlices [assembly]
